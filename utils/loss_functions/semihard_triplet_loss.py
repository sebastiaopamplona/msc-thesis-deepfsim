...
from utils.loss_functions.distance_functions import pairwise_distance
...
def masked_maximum(data, mask, dim=1):
    ...
    :return: masked_maximums: N-D `Tensor`.
...
def masked_minimum(data, mask, dim=1):
    ...
    :return: masked_minimums: N-D `Tensor`.
...
def adapted_semihard_triplet_loss(y_true, y_pred):
    ...
    semi_hard_triplet_loss_distance = math_ops.truediv(
        ...
        num_positives,
        name='adapted_semihard_triplet_loss')

    ### Code from Tensorflow function semi-hard triplet loss ENDS here.
    return semi_hard_triplet_loss_distance


def tf_semihard_triplet_loss(y_true, y_pred):
    """
    ...
    :param y_true: Dummy data; to be deleted.
    :param y_pred: [[labels], [embeddings]]

    :return triplet_loss: tf.float32 scalar.
    """
    del y_true
    margin = 1.
    labels = y_pred[:, :1]
